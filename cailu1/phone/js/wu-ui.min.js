(function() {
    ...
    Wu.prototype.showLoading = function(title) {
        ...
        var h = '<div class="wu-mask-transparent"></div>' + '<div class="wu-toast-box wu-toast-loading">' + $this.iosLoading + (title ? span : "") + "</div>";
        el.innerHTML = h;
    ...
    Wu.prototype.showLoadingBg = function() {
        ...
        var $this = this;
        var el = document.createElement("div");
        var h = '<div class="wu-mask-transparent bg-white"></div>' + '<div class="wu-toast-box wu-toast-box-transparent">' + $this.iosLoading + "</div>";
        el.innerHTML = h;
        el.className = "wu-toast";
        this.addDom(el);
        setTimeout(function() { el.classList.add("wu-animate-in") }, 0);
        return false
    };
    ...
    Wu.prototype.showError = function(title) {
        ...
        var el = document.createElement("div");
        el.innerHTML = '<div class="start-box-refresh"><div class="start-box-icon">' + '<i class="wu-icon icon-refresh"></i></div>' + '<p class="refresh-text">' + t + "<p>" + "</div>";
        el.className = "wu-toast";
        this.addDom(el);
        setTimeout(function() {
            el.classList.add("wu-animate-in")
    ...
    Wu.prototype.showToast = function(obj) {
        ...
        var h = (ismask ? mask : "") + '<div class="wu-toast-box">' + (icon ? iconLoad : "") + (title ? span : "") + "</div>";
        el.innerHTML = h;
        el.className = "wu-toast";
        this.hideToast();
        this.addDom(el);
    ...
    Wu.prototype.showDialog = function(ops) {
        ...
        var h = '<div class="wu-mask-transparent wu-mask-black"></div>' + '<div class="dialog-content">' + '<div class="dialog-title">' + title + "</div>" + '<div class="dialog-body">' + (showInput ? content_input : content_text) + "</div>" + '<div class="dialog-foot">' + '<a class="wu-btn dialog-btn-cancel" style="' + (showCancel ? "" : "display: none") + '">取消</a>' + '<a class="wu-btn dialog-btn-confirm">确认</a>' + "</div></div>";
    ...
    Wu.prototype.showAction = function(ops) {
        ...
        var el = document.createElement("div");
        h += '<div class="wu-mask-transparent wu-mask-black wu-mask-hide wu-mask-action"></div>' + '<div class="wu-actionsheet-main">' + '<div class="wu-actionsheet-title">' + title + "</div>" + '<div class="wu-actionsheet-menu">';
        if (menuArr.length) {
        ...
        if (cls.indexOf("dialog-btn-confirm") >= 0) {
            ...
            if (showInput) {
                ...
                if (typeof ops.success == "function") {
                    ops.success({ value: "confirm", inputValue: input_val })
                }
            } else {
                if (typeof ops.success == "function") {
                    ops.success({ value: "confirm" })
                }
            }
            ...
            if (val == "delete") {
                wu.showDialog({
                    ...
                    success: function(res) {
                        if (res.value == "confirm") { ops.success({ value: val, title: val_text }) }
                        that.handlerHide("actionsheet")
                    }
                })
            } else {
                if (typeof ops.success == "function") {
                    ops.success({ value: val, title: val_text });
                    that.handlerHide("actionsheet")
                }
            }
            return false
    ...
    Wu.prototype.handlerHide = function(type) {
        ...
        if (type == "dialog") {
            ...
            dialogView.addEventListener("webkitTransitionEnd", function() { this.remove() });
...
//打开toast
function openToast(titie, icon) {
    wu.showToast({
        title: titie,
        mask: false,
        icon: icon || "icon-success", // icon-success | icon-error | icon-info
        duration: 3000
    });
}
//打开顶部弹出的 消息框
...
//打开底部弹出菜单
function openActionsheet(title, json, dom, callback) {
    wu.showAction({
        title: title,
        deleteText: '你确定要删除吗？',
        menuArray: json,
        success: function(res) {
            ...
            if (res.value == "delete") {
                ...
                dom.val(res.title);
                dom.attr("data-value", res.value);
                callback && callback(res.title)
            }
        }
    })
}